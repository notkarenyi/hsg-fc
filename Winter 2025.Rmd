...
The overall budget for `r quarter` was `r count_x("totalreceived")`.

In `r quarter`, HSG Finance Committee funded `r nrow(summarize(group_by(df,org),n=n()))` HSOs, which reported at least `r count_x("attendactual")` attendees across `r count_x("eventsactual")` events in their post-quarter reviews. 
...
df <- df[, order(colnames(df))] %>%
  ...
  mutate(eventtotal = sum(amount, na.rm = T)) %>%
  ungroup() %>%
